...
<!-- 
	<c:if test="${page.prev}">
		<span>[ <a href="/board/listPageFirst?num=${page.startPageNum - 1}">이전</a> ]</span>
	</c:if>
	
	<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
		<span>
			<a href="/board/listPageFirst?num=${num}">${num}</a>
		</span>
	</c:forEach>
	
	<c:if test="${page.next}">
		<span>[ <a href="/board/listPageFirst?num=${page.endPageNum + 1}">다음</a> ]</span>
	</c:if>
-->
